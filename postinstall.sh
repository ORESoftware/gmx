...
cat run.gmx.sh > "$root_gmx" && chmod u+x "$root_gmx" || {
  echo "Could not write file or change permissions for file '$root_gmx'"
}

gmx_bin="$(npm bin -g)/gmx";
rm -rf "$gmx_bin" || echo "could not remove '$gmx_bin'";
cat run.gmx.sh > "$gmx_bin" && chmod u+x "$gmx_bin" || {
  echo "Could not write file or change permissions for file '$gmx_bin'"
}
